...
if $run_pcm_power; then
  ...
  echo "PCM-POWER"
  echo "----------------------------"
  log_debug "Preparing pcm-power with csv=/local/data/results/id_3_pcm_power.csv log=/local/data/results/id_3_pcm_power.log workload_cpu=6 profiler_cpu=5"
  idle_wait
  ...
  log_debug "Launching pcm-power workload (CSV=/local/data/results/id_3_pcm_power.csv, log=/local/data/results/id_3_pcm_power.log, profiler CPU=5, workload CPU=6)"
  sudo bash -lc '
    source /local/tools/compression_env/bin/activate
    cd /local/bci_code/id_3/code
