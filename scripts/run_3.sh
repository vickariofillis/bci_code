...
: "${TS_OUT:=/local/data/results/id_3_turbostat.txt}"  # update suffix per workload
...
PQOS_OUT="/local/data/results/id_3_pqos_core6.csv"  # use workload-specific filename
...
if $run_pcm_power; then
  ...
  echo "PCM-POWER"
  echo "----------------------------"
  log_debug "Launching pcm-power with turbostat+pqos sidecars (CSV=/local/data/results/id_3_pcm_power.csv, log=/local/data/results/id_3_pcm_power.log, profiler CPU=5, workload CPU=6)"
  idle_wait
