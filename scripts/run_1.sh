...
WORKLOAD_EXEC_SHELL="$(bci_wrap_command_for_placement_smoke "${WORKLOAD_EXEC_SHELL}")"
WORKLOAD_PLAIN_EXEC_SHELL="$(bci_wrap_command_for_placement_smoke "${WORKLOAD_PLAIN_EXEC_SHELL}")"
export ID1_BIN
...
BCI_WORKLOAD_ID="id_1"
BCI_WORKLOAD_MODE="${ID1_MODE}"
...
################################################################################
print_section "3. Change into the proper directory"
...
if $run_pcm || $run_pcm_memory || $run_pcm_power || $run_pcm_pcie; then
  ...
  if $run_pcm_pcie; then
    ...
    run_command_with_optional_mba_pid_tracking "${RESULT_PREFIX}_workload_pcm_pcie.log" bash -lc "${WORKLOAD_PLAIN_EXEC_SHELL}"
  ...
  if $run_pcm; then
    ...
    run_command_with_optional_mba_pid_tracking "${RESULT_PREFIX}_workload_pcm.log" bash -lc "${WORKLOAD_PLAIN_EXEC_SHELL}"
    if [[ -n ${PCM_PID:-} ]]; then
  ...
  pcm_mem_start=$(date +%s)
    ...
    run_command_with_optional_mba_pid_tracking "${RESULT_PREFIX}_workload_pcm_memory.log" bash -lc "${WORKLOAD_PLAIN_EXEC_SHELL}"
  ...
  run_command_with_optional_mba_pid_tracking "${RESULT_PREFIX}_workload_pcm_power.log" bash -lc "${WORKLOAD_PLAIN_EXEC_SHELL}"
  ...
  run_command_with_optional_mba_pid_tracking "${RESULT_PREFIX}_workload_pcm_memory_pass2.log" bash -lc "${WORKLOAD_PLAIN_EXEC_SHELL}"
...
run_command_with_optional_mba_pid_tracking "${MAYA_LOG_PATH}" cset proc --exec --set "${WORKLOAD_CPUSET_NAME}" -- env "${ID1_ENV_ASSIGNMENTS[@]}" taskset -c "${WORKLOAD_CPU}" "${ID1_BIN}" "${ID1_RUNTIME_ARGS[@]}" || workload_status=$?
...
if $run_toplev_basic; then
  ...
  bci_build_toplev_basic_command toplev_basic_cmd \
    ...
    "${WORKLOAD_EXEC_SHELL}" "${RESULT_PREFIX}_toplev_basic.log"
...
if $run_perf_evidence; then
  ...
  printf -v perf_evidence_cmd 'taskset -c %q perf stat -x, -o %q -e %q -- %s >>%q 2>&1' \
    "${TOOLS_CPU}" "${PERF_EVIDENCE_RAW_CSV}" "${PERF_EVIDENCE_EVENTS}" "${WORKLOAD_EXEC_SHELL}" "${PERF_EVIDENCE_LOG}"
