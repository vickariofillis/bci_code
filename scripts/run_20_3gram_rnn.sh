...
if [[ -n ${ID20_RNN_OUTPUT_PATH:-} ]]; then
  mkdir -p "$(dirname "${ID20_RNN_OUTPUT_PATH}")"
fi
...
BCI_WORKLOAD_ID="id_20_rnn"
BCI_WORKLOAD_MODE="${ID20_RNN_MODEL:-baseline}"
...
$run_toplev_basic || write_done_skipped "Toplev Basic" "${OUTDIR}/done_rnn_toplev_basic.log"
$run_toplev_full || write_done_skipped "Toplev Full" "${OUTDIR}/done_rnn_toplev_full.log"
$run_toplev_execution || \
  write_done_skipped "Toplev Execution" "${OUTDIR}/done_rnn_toplev_execution.log"
$run_perf_evidence || write_done_skipped "Perf Stat" "${OUTDIR}/done_rnn_perf_stat.log"
$run_maya || write_done_skipped "Maya" "${OUTDIR}/done_rnn_maya.log"
$run_pcm || write_done_skipped "PCM" "${OUTDIR}/done_rnn_pcm.log"
$run_pcm_memory || write_done_skipped "PCM Memory" "${OUTDIR}/done_rnn_pcm_memory.log"
$run_pcm_power || write_done_skipped "PCM Power" "${OUTDIR}/done_rnn_pcm_power.log"
$run_pcm_pcie || write_done_skipped "PCM PCIE" "${OUTDIR}/done_rnn_pcm_pcie.log"
    ...
    if resctrl_mon_should_use; then
      ...
      if start_resctrl_monitor "${RESCTRL_MON_INTERVAL_SEC:-${PQOS_INTERVAL_SEC:-0.5}}" "${RESCTRL_MON_CSV}" RESCTRL_MON_PID; then
        ...
        if (( resctrl_workload_rc != 0 )); then
          log_warn "Resctrl monitor workload run failed with exit ${resctrl_workload_rc}; see ${RESULT_PREFIX}_pqos_workload.log"
...
if $run_toplev_basic; then
  ...
  bci_build_toplev_basic_command toplev_basic_cmd \
    "${TOOLS_CPU}" "${TOPLEV_BASIC_INTERVAL_MS}" "${RESULT_PREFIX}_toplev_basic.csv" \
    "${workload_cmd}" "${RESULT_PREFIX}_toplev_basic.log"
  [[ -n "${toplev_basic_note}" ]] && echo "${toplev_basic_note}" >> "${RESULT_PREFIX}_toplev_basic.log"
  ...
  write_done_runtime "Toplev Basic" "$(secs_to_dhm "$toplev_basic_runtime")" "${OUTDIR}/done_rnn_toplev_basic.log"
  bci_register_collector_metadata \
    "toplev-basic" "toplev" "$(bci_metric_families_for_collector toplev-basic)" \
    "${RESULT_PREFIX}_toplev_basic.csv,${RESULT_PREFIX}_toplev_basic.log" \
...
if $run_toplev_execution; then
  ...
  write_done_runtime "Toplev Execution" "$(secs_to_dhm "$toplev_execution_runtime")" "${OUTDIR}/done_rnn_toplev_execution.log"
  bci_register_collector_metadata \
    "toplev-execution" "toplev" "$(bci_metric_families_for_collector toplev-execution)" \
    "${RESULT_PREFIX}_toplev_execution.csv,${RESULT_PREFIX}_toplev_execution.log" \
...
if $run_toplev_full; then
  ...
  write_done_runtime "Toplev Full" "$(secs_to_dhm "$toplev_full_runtime")" "${OUTDIR}/done_rnn_toplev_full.log"
  bci_register_collector_metadata \
    "toplev-full" "toplev" "$(bci_metric_families_for_collector toplev-full)" \
    "${RESULT_PREFIX}_toplev_full.csv,${RESULT_PREFIX}_toplev_full.log" \
...
if $run_perf_evidence; then
  ...
  write_done_runtime "Perf Stat" "$(secs_to_dhm "$perf_evidence_runtime")" "${OUTDIR}/done_rnn_perf_stat.log"
