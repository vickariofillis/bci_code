...
PQOS_TICKS="5"
PQOS_OUT="/local/data/results/id_20_3gram_lm_pqos_core6.csv"
...
$run_toplev_full || echo "Toplev-full run skipped" > /local/data/results/done_lm_toplev_full.log
...
if ! $run_pcm; then
  log_debug "PCM disabled -> populated /local/data/results/done_lm_pcm.log"
fi
if ! $run_pcm_memory; then
  log_debug "PCM-memory disabled -> populated /local/data/results/done_lm_pcm_memory.log"
fi
if ! $run_pcm_power; then
  log_debug "PCM-power disabled -> populated /local/data/results/done_lm_pcm_power.log"
fi
if ! $run_pcm_pcie; then
  log_debug "PCM-pcie disabled -> populated /local/data/results/done_lm_pcm_pcie.log"
fi
...
if $run_pcm_power; then
  ...
  echo "PCM-POWER"
  echo "----------------------------"
  log_debug "Preparing pcm-power with csv=/local/data/results/id_20_3gram_lm_pcm_power.csv log=/local/data/results/id_20_3gram_lm_pcm_power.log workload_cpu=6 profiler_cpu=5"
  idle_wait
  ...
  log_debug "Launching pcm-power workload (CSV=/local/data/results/id_20_3gram_lm_pcm_power.csv, log=/local/data/results/id_20_3gram_lm_pcm_power.log, profiler CPU=5, workload CPU=6)"
  sudo -E bash -lc '
    source /local/tools/bci_env/bin/activate
    export LD_LIBRARY_PATH="${LD_LIBRARY_PATH:-}"
    . path.sh
  ...
  export A_PCM="/local/data/results/id_20_3gram_lm_pcm_power.csv"
...
log_debug "Wrote /local/data/results/done_lm.log"

rm -f /local/data/results/done_lm_toplev_basic.log \
      /local/data/results/done_lm_toplev_full.log \
      /local/data/results/done_lm_toplev_execution.log \
      /local/data/results/done_lm_maya.log \
      /local/data/results/done_lm_pcm.log \
