  ...
  if resctrl_mon_should_use; then
    ...
    if start_resctrl_monitor "${RESCTRL_MON_INTERVAL_SEC:-${PQOS_INTERVAL_SEC:-0.5}}" "${RESCTRL_MON_CSV}" RESCTRL_MON_PID; then
      ...
      if (( pqos_workload_rc != 0 )); then
        log_warn "Resctrl monitor workload run failed with exit ${pqos_workload_rc}; see ${LM_PQOS_WORKLOAD_LOG}"
...
if $run_toplev_basic; then
  ...
  bci_toplev_basic_validate_csv "${LM_TOPLEV_BASIC_CSV}" "${toplev_basic_mode}"
