...
: "${TS_OUT:=/local/data/results/id_20_3gram_lm_turbostat.txt}"  # update suffix per workload
...
PQOS_OUT="/local/data/results/id_20_3gram_lm_pqos_core6.csv"  # use workload-specific filename
...
$run_pcm || log_debug "PCM disabled; wrote /local/data/results/done_lm_pcm.log"
$run_pcm_memory || echo "PCM-memory run skipped" > /local/data/results/done_lm_pcm_memory.log
$run_pcm_memory || log_debug "PCM-memory disabled; wrote /local/data/results/done_lm_pcm_memory.log"
$run_pcm_power || echo "PCM-power run skipped" > /local/data/results/done_lm_pcm_power.log
$run_pcm_power || log_debug "PCM-power disabled; wrote /local/data/results/done_lm_pcm_power.log"
$run_pcm_pcie || echo "PCM-pcie run skipped" > /local/data/results/done_lm_pcm_pcie.log
$run_pcm_pcie || log_debug "PCM-pcie disabled; wrote /local/data/results/done_lm_pcm_pcie.log"
...
if $run_pcm_power; then
  ...
  echo "PCM-POWER"
  echo "----------------------------"
  log_debug "Launching pcm-power with turbostat+pqos sidecars (CSV=/local/data/results/id_20_3gram_lm_pcm_power.csv, log=/local/data/results/id_20_3gram_lm_pcm_power.log, profiler CPU=5, workload CPU=6)"
  idle_wait
