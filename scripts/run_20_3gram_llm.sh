...
################################################################################
### run_id20_llm.sh
###   – Toplev + Maya profiling for the LLM workload
################################################################################
...
awk '{ for(i=1;i<=NF;i++){ printf "%s%s",$i,(i<NF?",":"") } print "" }' \
  /local/data/results/id_20_llm_maya.txt \
  > /local/data/results/id_20_llm_maya.csv

echo "LLM profiling complete; results in /local/data/results/"
