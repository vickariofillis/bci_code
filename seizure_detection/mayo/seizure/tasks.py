...
def split_train_ictal(X, y, latencies, cv_ratio):
    ...
    ## BCINOTE: tuple unpacking deprecated in Python 3. Goal is to sort list of tuples (e.g., [(a, b),...,(y, z)]) in descending order based on second element of each tuple (e.g., b and z)
    tagged_durations.sort(key=lambda x: x[1], reverse=True)
    ## BCINOTE: In Python 2.x, floating point division (/) is floor division when both args are int, whereas Python 3 returns a float. Changed the next 2 lines of code from / to // 
    ## BCINOTE: https://stackoverflow.com/questions/183853/what-is-the-difference-between-and-when-used-for-division
    ...
    ## BCINOTE: Sort list of tuples in ascending order based on first element of each tuple
